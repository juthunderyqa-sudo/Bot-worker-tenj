...
function understandIntentRuleBased(text) {
  const raw = String(text || '').trim();
  const lower = raw.toLowerCase();

  const budget = parseBudgetText(raw);
  if (budget) return { action: 'add_budget', confidence: 0.98, ...budget };

  const shopping = parseShoppingText(raw);
  if (shopping) return { action: 'add_shopping', confidence: 0.97, ...shopping };

  const note = parseNoteText(raw);
  if (note) return { action: 'add_note', confidence: 0.96, note_text: note };

  const explicitTask = /^(створи(ти)? задачу|додай задачу|нагадай мені|нагадай|нагадування|зустріч|запиши зустріч|подія)\b/i.test(raw);
  const hasTime = /\b\d{1,2}:\d{2}\b/.test(raw);
  const hasDateWord = /(сьогодні|завтра|післязавтра|\d{1,2}[./]\d{1,2}(?:[./]\d{2,4})?)/i.test(raw);
  if (explicitTask || hasTime || hasDateWord) {
    return { action: 'create_task', confidence: 0.95, ...parseTaskText(raw) };
  }

  if (/^(покажи|що в мене|мій план|план на сьогодні|сьогодні)\b/i.test(raw)) {
    return { action: 'today_overview', confidence: 0.92 };
  }
  if (/^(список задач|мої задачі|задачі)\b/i.test(raw)) {
    return { action: 'list_tasks', confidence: 0.92 };
  }
  if (/^(звіт за тиждень|тиждень)\b/i.test(raw)) {
    return { action: 'weekly_report', confidence: 0.9 };
  }
  if (/^(знайди|пошукай|хто такий|що таке|скільки|де купити|порівняй)\b/i.test(raw)) {
    return { action: 'search', confidence: 0.85, query: raw };
  }
  if (/^(місця поруч|знайди поруч|поруч)\b/i.test(raw)) {
    return { action: 'nearby_places', confidence: 0.85, query: raw.replace(/^(місця поруч|знайди поруч|поруч)\s*/i, '').trim() };
  }
  return { action: 'answer', confidence: 0.2, answer_text: raw };
}

function parseBudgetText(raw) {
  const lower = raw.toLowerCase();
  if (!/(витрат|дохід|зароб|прибут|income|expense)/i.test(lower)) return null;
  const type = /(дохід|зароб|прибут|income)/i.test(lower) ? 'income' : 'expense';
  const amountMatch = raw.match(/(-?\d+[\d\s.,]*)/);
  const amount = amountMatch ? String(amountMatch[1]).replace(/\s+/g, '') : '';
  const currencyMatch = raw.match(/(грн|uah|usd|eur|€|\$)/i);
  const currency = currencyMatch ? currencyMatch[1] : 'грн';
  let note = raw
    .replace(/^(додай\s+)?(витрат[ауи]?|дохід|заробіток|income|expense)\b[:\s-]*/i, '')
    .replace(/-?\d+[\d\s.,]*/,'')
    .replace(/(грн|uah|usd|eur|€|\$)/i,'')
    .trim();
  const category = note.split(/\s+/).slice(0,3).join(' ') || 'Інше';
  return { amount, currency, type, category, note: note || raw };
}

function parseShoppingText(raw) {
  let list_type = null;
  if (/^(додай у продукти|продукти:?|купити продукти|продукт[иі])\b/i.test(raw)) list_type = 'products';
  else if (/^(що купити|купи|додай у покупки|покупки:?|купити)\b/i.test(raw)) list_type = 'buy';
  if (!list_type) return null;
  let cleaned = raw
    .replace(/^(додай у продукти|продукти:?|купити продукти|продукт[иі]|що купити|купи|додай у покупки|покупки:?|купити)\b[:\s-]*/i, '')
    .trim();
  const qtyMatch = cleaned.match(/(\d+[\d.,]*\s*(шт|кг|г|л|мл|пач|уп|бут|pcs)?)$/i);
  const quantity = qtyMatch ? qtyMatch[1].trim() : '';
  if (qtyMatch) cleaned = cleaned.slice(0, qtyMatch.index).trim();
  const parts = cleaned.split(/\s+-\s+|,\s*/);
  return { list_type, item_name: parts[0] || cleaned || raw, details: parts.slice(1).join(', '), quantity };
}

function parseNoteText(raw) {
  const m = raw.match(/^(занотуй|нотатка|запиши нотатку|запиши)\b[:\s-]*(.+)$/i);
  return m ? m[2].trim() : null;
}

function parseTaskText(raw) {
  let title = raw.trim();
  title = title.replace(/^(створи(ти)? задачу|додай задачу|нагадай мені|нагадай|нагадування|зустріч|запиши зустріч|подія)\b[:\s-]*/i, '').trim();
  const parsed = parseDateTimeFromUkrText(title);
  let cleanTitle = title;
  if (parsed.matchedText) {
    cleanTitle = cleanTitle.replace(parsed.matchedText, ' ').replace(/\s+/g, ' ').trim();
  }
  cleanTitle = cleanTitle.replace(/^(про|що|щоб)\s+/i, '').trim();
  return {
...
function parseDateTimeFromUkrText(text) {
  const now = new Date();
  const offset = getTimeZoneOffsetString(DEFAULT_TIMEZONE, now);
  let baseDate = null;
  let matchedText = '';

  if (/післязавтра/i.test(text)) {
    baseDate = new Date(now.getTime() + 2 * 86400000);
    matchedText = 'післязавтра';
  } else if (/завтра/i.test(text)) {
    baseDate = new Date(now.getTime() + 86400000);
    matchedText = 'завтра';
  } else if (/сьогодні/i.test(text)) {
    baseDate = new Date(now);
    matchedText = 'сьогодні';
  }
  ...
  if (timeMatch) {
    ...
    matchedText = `${matchedText} ${timeMatch[0]}`.trim();
    return { due_at: `${yyyy}-${mon}-${dd}T${hh}:${mm}:00${offset}`, due_date: '', matchedText };
  }
  ...
  if (baseDate) {
    ...
    return { due_at: '', due_date: `${yyyy}-${mon}-${dd}`, matchedText };
  }

  return { due_at: '', due_date: '', matchedText: '' };
}
...
async function processAssistantInput(env, payload) {
  ...
  try {
    ...
    const answer = intent.answer_text && intent.answer_text !== normalizedText ? intent.answer_text : TEXT.FALLBACK;
...
async function understandIntent(env, text, history = []) {
  const quick = understandIntentRuleBased(text);
  if (quick.confidence >= 0.85) return quick;

  const timezone = env.TIMEZONE || DEFAULT_TIMEZONE;
  const historyText = history.slice(-6).map((item) => `${item.role}: ${item.text}`).join("\n");
  const prompt = [
    ...
    "Для create_task поверни: title, details, due_at, due_date.",
  ...
  try {
    const raw = await geminiText(env, {
      ...
      useSearch: false,
      timeoutMs: Number(env.GEMINI_INTENT_TIMEOUT_MS || 12000)
    });
    const parsed = JSON.parse(raw);
    if (!parsed.action) parsed.action = quick.action || 'answer';
    return { ...quick, ...parsed };
  } catch {
    return quick;
  }
...
async function runSearch(env, chatId, from, query, inputType) {
  ...
  let answer = '';
  try {
    answer = await answerWithGoogleSearch(env, query, history);
  } catch (error) {
    answer = 'Пошук зараз недоступний. Спробуй ще раз трохи пізніше або сформулюй запит коротше.';
  }
  await logAndReply(env, {
    chatId,
    from,
...
async function geminiText(env, { contents, responseMimeType = "text/plain", useSearch = false, timeoutMs = null }) {
  ...
  }, Number(timeoutMs || env.GEMINI_TIMEOUT_MS || 25000));
