...
const REG_CACHE_TTL = 60 * 60 * 24 * 30;
...
const FORM_STEPS = {
  ...
  WAITING_ADMIN_NAME: "waiting_admin_name",
  WAITING_ADMIN_PHONE: "waiting_admin_phone",
  WAITING_ADMIN_COMMENT: "waiting_admin_comment",
...
const CALLBACKS = {
  ...
  ADMIN_DAY_PICKER: "admin:day_picker",
  ADMIN_DAY_PREFIX: "admin:day:",
  ADMIN_OVERVIEW: "admin:overview",
  ADMIN_CREATE: "admin:create",
  ADMIN_REMOVE_PREFIX: "admin:remove:",
  ADMIN_REMOVE_CONFIRMED_PREFIX: "admin:remove_ok:",
...
const TEXT = {
  ...
  START_REGISTERED: "Вітаю 🌿 Оберіть потрібну дію нижче.",
  ...
  ADMIN_CREATE_START: "<b>➕ Створення запису адміністратором</b>\nВведіть ім'я клієнта.",
  ADMIN_CREATE_PHONE: "Введіть номер клієнта у форматі <b>380XXXXXXXXX</b>.",
  ADMIN_CREATE_COMMENT: "За потреби додайте коментар до запису або натисніть <b>Пропустити</b>.",
  ADMIN_CREATE_DONE: "Запис успішно створено адміністратором.",
  ...
  SKIP: "Пропустити",
  MY_BOOKINGS_TITLE: "<b>📂 Мої записи</b>",
  ADMIN_PENDING_TITLE: "<b>🆕 Нові заявки на підтвердження</b>",
  ADMIN_TODAY_TITLE: "<b>📅 Записи на сьогодні</b>",
  STATS_TITLE: "<b>📊 Короткий огляд</b>",
  ADMIN_HOME_TEXT: "<b>👑 Адмін-панель</b>\nОберіть, що саме зробити або подивитися.",
  DAY_PICKER_TITLE: "<b>🗓 Записи по днях</b>\nОберіть день, який хочете переглянути.",
  DAY_EMPTY: "На цей день записів немає.",
  ADMIN_REMOVE_PROMPT: "Видалити цей запис з розкладу?",
  ADMIN_REMOVE_DONE: "Запис прибрано з розкладу.",
...
async function handleMessage(message, env) {
  ...
  if (text === "/book" || text === "Записатися ✂️") {
    const state = await getState(env, chatId);
    if (state) {
  ...
  if (text === "/my_bookings" || text === "Мої записи 📂") {
    await clearState(env, chatId);
    await sendUserBookings(env, chatId, userId);
    return;
  }

  if (text === "/pending" || text === "Нові заявки 🆕") {
    if (!admin) {
  ...
  if (text === "/stats" || text === "Короткий огляд 📊") {
    if (!admin) {
      await sendMessage(env, chatId, TEXT.ADMIN_ONLY, { reply_markup: mainMenuKeyboard(true, false) });
      return;
    }
  ...
  if (text === "Створити запис ➕") {
    ...
    await startAdminCreateFlow(env, chatId);
    return;
  }
...
async function processRegisteredFlow(message, state, env, registration) {
  ...
  switch (state.step) {
    ...
    case FORM_STEPS.WAITING_ADMIN_NAME: {
      if (!validateName(text)) {
        await sendMessage(env, chatId, TEXT.INVALID_NAME, { reply_markup: cancelKeyboard() });
        return;
      }
      data.full_name = text;
      await saveState(env, chatId, { step: FORM_STEPS.WAITING_ADMIN_PHONE, data });
      await sendMessage(env, chatId, TEXT.ADMIN_CREATE_PHONE, { reply_markup: cancelKeyboard() });
      return;
    }

    case FORM_STEPS.WAITING_ADMIN_PHONE: {
      const phone = normalizePhone(text);
      if (!validatePhone(phone)) {
        await sendMessage(env, chatId, TEXT.INVALID_PHONE, { reply_markup: cancelKeyboard() });
        return;
      ...
      data.phone = phone;
      await saveState(env, chatId, { step: FORM_STEPS.WAITING_BOOKING_DATE, data: { ...data, flow_type: "admin_create" } });
      const dates = getUpcomingDates(env, BOOKING_LOOKAHEAD_DAYS);
      await sendMessage(env, chatId, TEXT.START_BOOKING, { reply_markup: bookingDatesKeyboard(dates, true) });
      return;
    }

    case FORM_STEPS.WAITING_ADMIN_COMMENT: {
      const comment = text === TEXT.SKIP ? "" : text;
      if (comment.length > MAX_COMMENT_LENGTH) {
        await sendMessage(env, chatId, TEXT.COMMENT_TOO_LONG, { reply_markup: skipKeyboard() });
        return;
      ...
      data.comment = comment;
      await sendAdminBookingConfirmation(env, chatId, data, message.from);
      return;
    }
...
async function startAdminCreateFlow(env, chatId) {
  await saveState(env, chatId, { step: FORM_STEPS.WAITING_ADMIN_NAME, data: { flow_type: "admin_create" } });
  await sendMessage(env, chatId, TEXT.ADMIN_CREATE_START, { reply_markup: cancelKeyboard() });
}
...
async function handleCallbackQuery(callbackQuery, env) {
  ...
  try {
    ...
    if (data === CALLBACKS.ADMIN_HOME) {
      if (!admin) {
        await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_ONLY, true);
        return;
      }
      await sendAdminHome(env, chatId);
      await answerCallbackQuery(env, callbackQuery.id, "Відкриваю адмін-панель.", false);
      return;
    }

    if (data === CALLBACKS.ADMIN_DAY_PICKER) {
      if (!admin) {
        await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_ONLY, true);
        return;
      }
      await sendAdminDayPicker(env, chatId);
      await answerCallbackQuery(env, callbackQuery.id, "Оберіть день.", false);
      return;
    }

    if (data === CALLBACKS.ADMIN_OVERVIEW) {
      if (!admin) {
        await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_ONLY, true);
        return;
      }
      await sendStats(env, chatId);
      await answerCallbackQuery(env, callbackQuery.id, "Показую короткий огляд.", false);
      return;
    }

    if (data === CALLBACKS.ADMIN_CREATE) {
      if (!admin) {
        await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_ONLY, true);
        return;
      }
      await startAdminCreateFlow(env, chatId);
      await answerCallbackQuery(env, callbackQuery.id, "Створюємо запис вручну.", false);
      return;
    }

    if (data.startsWith(CALLBACKS.ADMIN_DAY_PREFIX)) {
      if (!admin) {
        await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_ONLY, true);
        return;
      }
      const isoDate = data.slice(CALLBACKS.ADMIN_DAY_PREFIX.length);
      await sendBookingsForDate(env, chatId, isoDate);
      await answerCallbackQuery(env, callbackQuery.id, `Показую ${formatHumanDate(isoDate)}.`, false);
      return;
    }
    ...
    if (data.startsWith(CALLBACKS.ADMIN_REMOVE_PREFIX) || data.startsWith(CALLBACKS.ADMIN_REMOVE_CONFIRMED_PREFIX)) {
      await handleAdminRemove(callbackQuery, env);
      return;
    }
...
async function handleDateSelection(callbackQuery, env) {
  ...
  await saveState(env, chatId, {
    ...
    data: { ...(currentState?.data || {}), booking_date: selectedDate }
  });
...
async function handleTimeSelection(callbackQuery, env) {
  ...
  const nextStep = currentData.flow_type === "admin_create" ? FORM_STEPS.WAITING_ADMIN_COMMENT : FORM_STEPS.WAITING_BOOKING_COMMENT;

  await saveState(env, chatId, {
    step: nextStep,
    data: { ...currentData, booking_date: date, booking_time: time, slot_key: `${date}|${time}` }
  });
...
async function handleBookingConfirm(callbackQuery, env) {
  ...
  const registration = await getUserRegistration(env, userId);
  const draft = state.data || {};
  const isAdminCreate = state.step === FORM_STEPS.WAITING_ADMIN_CONFIRM || draft.flow_type === "admin_create";

  if (!isAdminCreate && (!registration?.is_registered || registration.is_registered !== "true")) {
    await answerCallbackQuery(env, callbackQuery.id, TEXT.NOT_REGISTERED, true);
    return;
  }
  ...
  const booking = {
    ...
    created_at: now,
    updated_at: now,
    telegram_id: isAdminCreate ? "" : String(userId || chatId),
    username: isAdminCreate ? `Адмін: ${formatAdminLabel(callbackQuery.from)}` : (registration.username || getTelegramUsername(callbackQuery.from)),
    full_name: isAdminCreate ? (draft.full_name || "") : (registration.full_name || ""),
    phone: isAdminCreate ? (draft.phone || "") : (registration.phone || ""),
    booking_date: draft.booking_date,
    booking_time: draft.booking_time,
    slot_key: `${draft.booking_date}|${draft.booking_time}`,
    comment: draft.comment || "",
    status: isAdminCreate ? STATUS_APPROVED : STATUS_PENDING,
    admin_action_by: isAdminCreate ? formatAdminLabel(callbackQuery.from) : "",
    approved_at: isAdminCreate ? now : "",
    rejected_at: "",
    cancelled_at: "",
    cancelled_by: ""
  };
  ...
  const successText = isAdminCreate ? TEXT.ADMIN_CREATE_DONE : TEXT.BOOKING_SAVED;
  await answerCallbackQuery(env, callbackQuery.id, successText, false);
  await sendMessage(env, chatId, successText, { reply_markup: mainMenuKeyboard(true, isAdmin(env, userId)) });
}
...
async function handleAdminRemove(callbackQuery, env) {
  ...
  const isConfirmed = callbackQuery.data.startsWith(CALLBACKS.ADMIN_REMOVE_CONFIRMED_PREFIX);
  const bookingId = callbackQuery.data
    .replace(CALLBACKS.ADMIN_REMOVE_CONFIRMED_PREFIX, "")
    .replace(CALLBACKS.ADMIN_REMOVE_PREFIX, "");
  const booking = await findBookingById(env, bookingId);

  if (!booking) {
    await answerCallbackQuery(env, callbackQuery.id, "Запис не знайдено.", true);
    return;
  }

  if (!isConfirmed) {
    await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_REMOVE_PROMPT, false);
    await sendMessage(env, callbackQuery.message.chat.id, formatAdminBooking(booking), {
      reply_markup: adminRemoveConfirmKeyboard(booking.booking_id)
    });
    return;
  }

  if (![STATUS_PENDING, STATUS_APPROVED].includes(booking.status || "")) {
    await answerCallbackQuery(env, callbackQuery.id, "Цей запис уже неактивний.", true);
    return;
  }
  ...
  await notifyUserAboutDecision(env, { ...booking, cancelled_at: now, cancelled_by: adminLabel }, STATUS_CANCELLED);

  await answerCallbackQuery(env, callbackQuery.id, TEXT.ADMIN_REMOVE_DONE, false);
  await sendMessage(env, callbackQuery.message.chat.id, `${TEXT.ADMIN_REMOVE_DONE}

${formatAdminBooking({ ...booking, status: STATUS_CANCELLED, admin_action_by: adminLabel, cancelled_at: now, cancelled_by: adminLabel })}`, {
...
async function handleAdminDecision(callbackQuery, env) {
  ...
  const current = refreshed || { ...booking, status: nextStatus, ...extra };
  await telegramApi(env, "editMessageText", {
    chat_id: callbackQuery.message.chat.id,
    message_id: callbackQuery.message.message_id,
    text: formatAdminBooking(current),
    parse_mode: "HTML",
    reply_markup: approve ? adminApprovedKeyboard(current.booking_id) : adminMenuInlineKeyboard()
  });
...
function mainMenuKeyboard(isRegistered, isAdminUser = false) {
  ...
  if (isAdminUser) {
    keyboard.push([{ text: "Створити запис ➕" }]);
    keyboard.push([{ text: "Адмін-панель 👑" }]);
  }
  keyboard.push([{ text: "Головне меню" }]);
  return { keyboard, resize_keyboard: true, input_field_placeholder: "Оберіть кнопку нижче" };
}

function mainMenuInlineKeyboard(isAdminUser = false) {
  const rows = [
    [{ text: "✂️ Записатися", callback_data: CALLBACKS.START_BOOKING }],
    [{ text: "📂 Мої записи", callback_data: CALLBACKS.MY_BOOKINGS }],
    [{ text: "🏠 Головне меню", callback_data: CALLBACKS.BACK_MAIN }]
  ];
  if (isAdminUser) {
    rows.push([{ text: "➕ Створити запис", callback_data: CALLBACKS.ADMIN_CREATE }]);
    rows.push([{ text: "👑 Адмін-панель", callback_data: CALLBACKS.ADMIN_HOME }]);
  }
...
function adminMenuInlineKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: "🗓 По днях", callback_data: CALLBACKS.ADMIN_DAY_PICKER }],
      [{ text: "➕ Створити запис", callback_data: CALLBACKS.ADMIN_CREATE }],
      [{ text: "📊 Короткий огляд", callback_data: CALLBACKS.ADMIN_OVERVIEW }],
      [{ text: "🏠 Головне меню", callback_data: CALLBACKS.BACK_MAIN }]
    ]
  };
}
...
function adminManageKeyboard(row) {
  if ((row.status || "") === STATUS_PENDING) {
    return adminDecisionKeyboard(row.booking_id);
  }
  if ((row.status || "") === STATUS_APPROVED) {
    return adminApprovedKeyboard(row.booking_id);
  }
  return adminMenuInlineKeyboard();
}

function adminApprovedKeyboard(bookingId) {
  return {
    inline_keyboard: [
      [{ text: "🗑 Прибрати запис", callback_data: `${CALLBACKS.ADMIN_REMOVE_PREFIX}${bookingId}` }],
      [{ text: "👑 До адмін-панелі", callback_data: CALLBACKS.ADMIN_HOME }]
    ]
  };
}

function adminRemoveConfirmKeyboard(bookingId) {
  return {
    inline_keyboard: [
      [{ text: "🗑 Так, прибрати", callback_data: `${CALLBACKS.ADMIN_REMOVE_CONFIRMED_PREFIX}${bookingId}` }],
      [{ text: "⬅️ Назад", callback_data: CALLBACKS.ADMIN_HOME }]
    ]
  };
}
...
async function sendAdminHome(env, chatId) {
  await sendMessage(env, chatId, TEXT.ADMIN_HOME_TEXT, { reply_markup: adminMenuInlineKeyboard() });
}

async function sendAdminDayPicker(env, chatId) {
  const days = getUpcomingDates(env, 10);
  await sendMessage(env, chatId, TEXT.DAY_PICKER_TITLE, { reply_markup: adminDaysKeyboard(days) });
}

async function sendBookingsForDate(env, chatId, isoDate) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(isoDate || ""))) {
    await sendMessage(env, chatId, TEXT.INVALID_DATE, { reply_markup: adminMenuInlineKeyboard() });
    return;
  }

  const rows = (await getBookings(env))
    .filter((row) => row.booking_date === isoDate)
    .filter((row) => [STATUS_PENDING, STATUS_APPROVED].includes(row.status || ""))
    .sort(compareBookingsAsc);

  if (!rows.length) {
    await sendMessage(env, chatId, `${TEXT.DAY_EMPTY}
📅 ${escapeHtml(formatHumanDate(isoDate))}`, { reply_markup: adminMenuInlineKeyboard() });
    return;
  }

  const header = `<b>🗓 Записи на ${escapeHtml(formatHumanDate(isoDate))}</b>
Кількість: <b>${rows.length}</b>`;
  await sendMessage(env, chatId, header, { reply_markup: adminMenuInlineKeyboard() });
  await sendChunkedText(env, chatId, rows.map(formatTodayLine).join("\n\n"), mainMenuKeyboard(true, true));
}

function adminDaysKeyboard(days) {
  const rows = [];
  for (let i = 0; i < days.length; i += 2) {
    rows.push(days.slice(i, i + 2).map((day) => ({
      text: `${day.weekday} ${day.human}`,
      callback_data: `${CALLBACKS.ADMIN_DAY_PREFIX}${day.iso}`
    })));
  }
  rows.push([{ text: "🏠 Головне меню", callback_data: CALLBACKS.BACK_MAIN }]);
  return { inline_keyboard: rows };
}
...
function getUpcomingDates(env, days) {
  const dates = [];
  const now = new Date();
  const workingDays = getWorkingDays(env);
  for (let i = 0; i < days * 3 && dates.length < days; i++) {
    const date = new Date(now.getTime());
    date.setDate(now.getDate() + i);
    const weekdayNumber = getWeekdayNumber(env, date);
    if (!workingDays.has(weekdayNumber)) continue;
...
function getWorkingDays(env) {
  const raw = String(env.WORKING_DAYS || DEFAULT_WORKING_DAYS);
  const values = raw.split(",").map((item) => Number(item.trim())).filter((n) => Number.isInteger(n) && n >= 1 && n <= 7);
  return new Set(values.length ? values : [1, 2, 3, 4, 5, 6]);
}

function getWeekdayNumber(env, date) {
...
async function updateBookingStatus(env, bookingId, status, extra = {}) {
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    await withGoogleRetry(() => sheetsUpdateValues(env, `${getBookingsSheetName(env)}!A${rowNumber}:${indexToColumn(headers.length - 1)}${rowNumber}`, [row]));
    return true;
  }
  return false;
}
...
async function getUserRegistration(env, telegramId) {
  const cacheKey = `userreg:${telegramId}`;
  const cached = await env.STATE_KV.get(cacheKey, { type: "json" });
  if (cached && String(cached.telegram_id || "") === String(telegramId || "") && String(cached.is_registered || "").toLowerCase() === "true") {
    return cached;
  }
  ...
  const result = pool.sort((a, b) => parseDateTimeValue(b.last_login_at || b.registered_at) - parseDateTimeValue(a.last_login_at || a.registered_at))[0] || null;

  if (result && String(result.is_registered || "").toLowerCase() === "true") {
    await env.STATE_KV.put(cacheKey, JSON.stringify(result), { expirationTtl: REG_CACHE_TTL });
  }
...
async function upsertUser(env, user) {
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    await withGoogleRetry(() => sheetsUpdateValues(env, `${getUsersSheetName(env)}!A${rowNumber}:${indexToColumn(headers.length - 1)}${rowNumber}`, [row]));
    if (String(merged.is_registered || "").toLowerCase() === "true") {
      await env.STATE_KV.put(`userreg:${merged.telegram_id}`, JSON.stringify(merged), { expirationTtl: REG_CACHE_TTL });
    }
    return;
  }
  ...
  if (String(user.is_registered || "").toLowerCase() === "true") {
    await env.STATE_KV.put(`userreg:${user.telegram_id}`, JSON.stringify(user), { expirationTtl: REG_CACHE_TTL });
  }
}
